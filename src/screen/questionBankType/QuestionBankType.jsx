...
const QuestionBankType = () => {
    ...
    const onAddType = () => navigate("/add-question-bank-type");
    const onEditType = (id) => navigate(`/edit-exam-type/${id}`); // Note: Keeping original navigation logic
    ...
    const filteredData = questionBankTypeList.data?.filter(item =>
        ...
        <div className="pb-5 text-white">
            ...
            <Container fluid className="mt-n4">
                ...
                <div className="d-flex justify-content-between align-items-center mb-4">
                    <div>
                        <h2 className="fw-bold mb-0 text-white">Question Bank Types</h2>
                        <p className="text-secondary small mb-0">Categorize your question banks for better organization</p>
                    </div>
                    <Button
                        onClick={onAddType}
                ...
                {/* Toolbar */}
                <Card className="bg-dark border-secondary shadow-sm mb-4" >
                                ...
                                <Table hover variant="dark" className="mb-0 align-middle custom-table">
                                    <thead className="bg-black">
                                        <tr className="text-secondary small text-uppercase">
                                            ...
                                            <th className="px-4 py-3 border-secondary">Type Name</th>
                                        ...
                                        {questionBankTypeList.loading ? (
                                            <tr>
                                                <td colSpan="4" className="text-center py-5">
                                                    ...
                                                    <span className="text-secondary">Loading types...</span>
                                                </td>
                                            </tr>
                                        ) : filteredData.length === 0 ? (
                                            <tr>
                                                    <td colSpan="4" className="text-center py-5 text-secondary">
                                                        No question bank types found
                                                    </td>
                                                </tr>
                                            ) : (
                                                filteredData.map((item) => (
                                                    <tr key={item.id} className="border-secondary">
                                                        <td className="px-4 py-3">
                                                            <Badge className="text-whiteborder border-secondary border-opacity-25 fw-normal">
                                                                #{item.id}
                                                            </Badge>
                                                        </td>
                                                        <td className="px-4 py-3">
                                                            <div className="d-flex align-items-center gap-3">
                                                                <div className="rounded bg-info bg-opacity-10 d-flex align-items-center justify-content-center text-white" style={{ width: '35px', height: '35px' }}>
                                                                    <FiLayers size={18} />
                                                                </div>
                                                        ...
                                                        <td className="px-4 py-3 text-end">
                                                            <Dropdown align="end">
                                                                <Dropdown.Toggle variant="link" className="text-secondary p-0 border-0 no-caret shadow-none bg-transparent text-bold">
                                                                    <FiMoreVertical />
                                                                </Dropdown.Toggle>
                                                                <Dropdown.Menu className="border-secondary shadow border border-opacity-25">
                                                                    <Dropdown.Item onClick={() => onEditType(item.id)}>
                                                                        <FiEdit2 className="me-2 text-info" /> Edit Info
                                                                    </Dropdown.Item>
                                                                    <Dropdown.Divider className="bg-secondary opacity-25" />
                                                                    <Dropdown.Item className="text-danger" onClick={() => onDeleteType(item.id)}>
